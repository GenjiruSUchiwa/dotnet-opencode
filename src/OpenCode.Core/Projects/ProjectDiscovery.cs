...
public static class ProjectDiscovery
{
    ...
    {
        var location = await CatalogLocation.ResolveIdentityAsync(database, directory, workspace, ct);
        await RecordAsync(database, location, ct);
        return location;
    }
    ...
    public static async Task RecordAsync(IDatabase database, LocationInfo location, CancellationToken ct = default)
    {
        ...
        await WorktreeService.RequirePluginsAsync(location.Directory, ct);
        ...
        if (!git && !hg) return;
        var cache = git ? (await new GitWorktreeStrategy().DiscoverAsync(root, ct)).CommonDirectory : Path.Combine(root, ".hg");
        var previous = "global";
        try
        {
            var text = (await File.ReadAllTextAsync(Path.Combine(cache, "opencode"), ct)).Trim();
            if (text.Length > 0) previous = text;
        }
        catch (Exception error) when (error is IOException or UnauthorizedAccessException) { }
        await ReconcileCanonicalAsync(database, location, ct);
        ...
        {
            var adopted = await MarkerlessProjectsAsync(database, location.Project.Id, item.Directory, ct);
            await new EventStore(database).TransactAsync(location.Project.Id.Value, async (transaction, token) =>
            {
                ...
                if (await transaction.Db.Set<WorktreeRow>().AnyAsync(row => row.project_id == location.Project.Id.Value && row.directory == directory, token)) return false;
                await transaction.AppendAsync(Resolved, new WorktreeResolvedEventData(location.Project.Id, item.Directory, ProjectId.FromExisting(previous),
                    adopted.Count == 0 ? null : adopted), token);
    ...
    {
        var stored = await new WorktreeStore(database).PrimaryAsync(location.Project.Id, ct);
        ...
        var project = await database.RunInTransactionAsync(async (connection, transaction) =>
        {
            ...
            await db.Set<ProjectRow>().Where(row => row.id == location.Project.Id.Value).ExecuteUpdateAsync(setters => setters
                .SetProperty(row => row.worktree, canonical).SetProperty(row => row.time_updated, updated), ct);
            var row = await db.Set<ProjectRow>().FirstOrDefaultAsync(row => row.id == location.Project.Id.Value, ct);
    ...
    private static async Task<IReadOnlyList<ProjectId>> MarkerlessProjectsAsync(IDatabase database, ProjectId project, string directory, CancellationToken ct)
    {
        ...
        var candidates = (await db.Set<ProjectRow>().Where(row => row.vcs == null && string.Compare(row.worktree, start) >= 0 && string.Compare(row.worktree, end) <= 0)
            .Select(row => new { row.id, row.worktree }).ToListAsync(ct)).Select(row => (Id: ProjectId.FromExisting(row.id), Directory: ProjectPaths.Platform(row.worktree))).ToArray();
    ...
    private static async Task ProjectAsync(EventTransaction transaction, OpenCodeEvent committed, CancellationToken ct)
    {
        ...
        var storedRows = await transaction.Db.Sessions.Where(row => row.workspace_id == null)
            .Join(transaction.Db.Set<ProjectRow>(), session => session.project_id, project => project.id,
                (session, project) => new { session.id, session.directory, session.project_id, project.worktree }).ToListAsync(ct);
        ...
        foreach (var row in rows)
        {
            ...
            await transaction.Db.Sessions.Where(session => session.id == row.Id.Value).ExecuteUpdateAsync(setters => setters
                .SetProperty(session => session.project_id, data.ProjectId.Value).SetProperty(session => session.path, path), ct);
        }
    }
