...
public sealed class DurableEventLog(IDatabase database, int pageSize = 512)
{
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        var target = await LatestAsync(aggregateId, ct).ConfigureAwait(true);
        var cursor = after;
        await foreach (var item in ReadThroughAsync(target, ct).ConfigureAwait(true)) yield return new DurableLogItem.Entry(item);
        yield return new DurableLogItem.Synced(aggregateId, target >= 0 ? target : null);
        if (subscription is null) yield break;
        await foreach (var _ in subscription.Wake.Reader.ReadAllAsync(ct).ConfigureAwait(true))
        {
            var through = await LatestAsync(aggregateId, ct).ConfigureAwait(true);
            if (through <= cursor) continue;
            await foreach (var item in ReadThroughAsync(through, ct).ConfigureAwait(true)) yield return new DurableLogItem.Entry(item);
        }
        ...
        async IAsyncEnumerable<OpenCodeEvent> ReadThroughAsync(long through, [EnumeratorCancellation] CancellationToken token)
        {
            while (true)
            {
                var page = await ReadPageAsync(aggregateId, cursor, through, token).ConfigureAwait(true);
    ...
    public Task ClaimAsync(string aggregateId, string ownerId, CancellationToken ct = default)
    {
        ...
        return new EventStore(database).TransactAsync(aggregateId, async (transaction, token) =>
        {
            return await transaction.Db.Sequences.Where(row => row.aggregate_id == aggregateId)
                .ExecuteUpdateAsync(setters => setters.SetProperty(row => row.owner_id, ownerId), token).ConfigureAwait(true); // Unknown aggregate: update-only no-op.
        }, ct);
    }
    ...
    public Task ReplayAsync(SerializedDurableEvent input, DurableReplayOptions? options = null, CancellationToken ct = default)
    {
        ...
        {
            var sequence = await transaction.Db.Sequences.FirstOrDefaultAsync(row => row.aggregate_id == input.AggregateId, token).ConfigureAwait(true);
            ...
            if (input.Seq <= latest)
            {
                var row = await transaction.Db.Events.Where(row => row.aggregate_id == input.AggregateId && SqliteFunctions.At(row.seq, input.Seq))
                    .Select(row => new { row.id, row.type, row.created, row.data }).FirstOrDefaultAsync(token).ConfigureAwait(true);
                var identical = false;
                if (row is not null)
                {
                ...
                if (!string.IsNullOrEmpty(replay.OwnerId) && owner is null)
                {
                    await transaction.Db.Sequences.Where(row => row.aggregate_id == input.AggregateId && row.owner_id == null)
                        .ExecuteUpdateAsync(setters => setters.SetProperty(row => row.owner_id, replay.OwnerId), token).ConfigureAwait(true);
                }
            ...
            var duplicate = await transaction.Db.Events.Where(row => row.id == input.Id.Value)
                .Select(row => new { row.aggregate_id, row.seq }).FirstOrDefaultAsync(token).ConfigureAwait(true);
            ...
            try { await decoded.Definition!.Project(transaction, decoded.Event, token).ConfigureAwait(true); }
            ...
            await SqliteIntrinsics.ReserveReplayAsync(transaction.Db, input.AggregateId, checked((long)input.Seq), replay.OwnerId,
                !string.IsNullOrEmpty(replay.OwnerId) && owner is null, token).ConfigureAwait(true);
            await transaction.Db.InsertAsync(new EventRow { id = input.Id.Value, aggregate_id = input.AggregateId,
                seq = checked((long)input.Seq), created = decoded.Event.Created, type = input.Type, data = decoded.Event.Data.GetRawText() }, token).ConfigureAwait(true);
    ...
    private async Task<IReadOnlyList<SerializedDurableEvent>> ReadPageAsync(string aggregateId, double after, long through, CancellationToken ct)
    {
        ...
        var db = new PersistenceContext(connection);
        await using var dbLifetime = db.ConfigureAwait(true);
        var rows = db.Events.Where(row => row.aggregate_id == aggregateId && SqliteFunctions.After(row.seq, after) && row.seq <= through)
            .OrderBy(row => row.seq).Select(row => new { row.id, row.type, row.seq, row.created, row.data });
        var result = new List<SerializedDurableEvent>();
        await foreach (var row in rows.ReadAsync(pageSize, ct).ConfigureAwait(true))
        {
