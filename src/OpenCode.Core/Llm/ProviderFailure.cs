...
{
    // JavaScript \s is not .NET ECMAScript's ASCII-only \s, or .NET's Unicode
    // whitespace set (which includes U+0085 and omits U+FEFF).
    ...
    private static readonly Regex[] ContextOverflow = new[]
    {
        ...
        @"exceeds (?:the )?(?:model'?s )?maximum context length(?: of [0-9,]+ tokens?|[" + Space + @"]*\([0-9,]+\))",
        @"input token count[^\r\n\u2028\u2029]*exceeds the maximum", @"tokens in request more than max tokens allowed",
        ...
        @"context length is only [0-9]+ tokens", @"input length[^\r\n\u2028\u2029]*exceeds[^\r\n\u2028\u2029]*context length",
        ...
        @"token limit exceeded", @"request_too_large", @"^4(?:00|13)[" + Space + @"]*(?:status code)?[" + Space + @"]*\(no body\)"
    ...
    private static readonly Regex ContentPolicy = Pattern(@"content[-_" + Space + @"]?policy|content_filter|safety");
    private static readonly Regex Quota = Pattern(@"insufficient[-_" + Space + @"]?quota|quota[-_" + Space + @"]?exceeded");
    private static readonly Regex RateLimit = Pattern(@"rate increased too quickly|rate[-_" + Space + @"]?limit|too[_" + Space + @"]?many[_" + Space + @"]?requests");
#pragma warning disable MA0009 // Fixed/bounded alternatives scan linearly; retain ECMAScript word-boundary semantics, which NonBacktracking cannot express.
    private static readonly Regex ServerError = new(@"\b(?:try again|(?:please |you can )?retry (?:the |this |your )?request|try (?:the |this |your )?request again|(?:currently |temporarily )?at capacity|overloaded|temporarily unavailable|service[-_" + Space + @"]?unavailable|(?:server|internal)[-_" + Space + @"]?error|server (?:is )?busy|provider returned (?:an )?error|resource[-_" + Space + @"]?exhausted|upstream (?:connect|connection|request)|request buffer limit while retrying upstream)\b", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.ECMAScript);
#pragma warning restore MA0009
    ...
    {
        var codes = Codes(data, decodeString: true).Concat(Codes(Parse(body))).Concat(Codes(Parse(message)))
    ...
    private static IEnumerable<string> Codes(JsonElement value, bool decodeString = false)
    {
        // providerCodes decodes string-valued data once. Raw body/message inputs
        // have already been decoded, so nested JSON strings must not be decoded again.
        var decoded = decodeString && value.ValueKind == JsonValueKind.String ? Parse(value.GetString()) : value;
        var error = Get(decoded, "error");
        var responseError = Get(Get(decoded, "response"), "error");
        var exception = Get(decoded, "exception");
        return new[] { Get(decoded, "code"), Get(error, "code"), Get(error, "type"), Get(error, "status"),
    ...
    // Explicit classes match JavaScript digits, whitespace and line terminators;
    // ordinary .NET \d/\s/dot would change retry-facing failure evidence.
