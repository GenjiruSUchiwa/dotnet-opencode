...
public sealed class McpOAuthCredentialStore(CredentialStore store, Func<CredentialMutation, CancellationToken, Task> publish) : IMcpOAuthCredentials
{
    public async Task<CredentialInfo?> SelectedAsync(string integrationId, CancellationToken ct) =>
        Convert(await store.GetActiveCredentialAsync(integrationId, ct));

    public async Task<CredentialInfo?> GetAsync(CredentialId id, CancellationToken ct) =>
        Convert(await store.GetCredentialAsync(id.Value, ct));

    public async Task<CredentialId> CreateAsync(string integrationId, CredentialOAuth value, string? label, CancellationToken ct)
    {
        var mutation = await store.CreateAsync(integrationId, JsonSerializer.SerializeToElement(value, OpenCodeJsonContext.Default.CredentialValue), label, ct);
    ...
    {
        var mutation = await store.UpdateAsync(id.Value, value: JsonSerializer.SerializeToElement(value, OpenCodeJsonContext.Default.CredentialValue), ct: ct);
        await publish(mutation, CancellationToken.None);
    ...
    public async Task<bool> RemoveAsync(CredentialId id, string? expectedRefresh, CancellationToken ct)
    {
        ...
        var mutation = await store.RemoveAsync(id.Value, ct);
        await publish(mutation, CancellationToken.None);
    ...
    public async ValueTask StoreTokensAsync(TokenContainer tokens, CancellationToken cancellationToken = default)
    {
        if (tokens is null)
        {
            ...
            if (Volatile.Read(ref _presentedRefresh) is { } refresh)
                await credentials.RemoveAsync(id, refresh, cancellationToken);
            return;
        }
        if (await ReadAsync(cancellationToken) is null)
            throw new McpOAuthRequiredException();
        var value = McpOAuthService.ToCredential(integrationId, serverUrl, tokens);
        if (!await credentials.UpdateAsync(id, value, cancellationToken)) throw new McpOAuthRequiredException();
        Volatile.Write(ref _presentedRefresh, value.Refresh);
    }

    private async Task<CredentialOAuth?> ReadAsync(CancellationToken ct)
    {
        var current = await credentials.GetAsync(id, ct);
        if (current is null) return null;
