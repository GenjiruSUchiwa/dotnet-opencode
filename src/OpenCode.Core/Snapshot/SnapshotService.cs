...
public sealed class SessionSnapshotLocations(IDatabase database, string? dataDirectory = null, string gitExecutable = "git")
{
    ...
    {
        try { return await GetAsync(location, ct); }
    ...
    public async Task<SnapshotService> GetAsync(LocationRef location, CancellationToken ct = default)
    {
        ...
        var info = await CatalogLocation.ResolveAsync(database, key.Directory, key.WorkspaceId?.Value, ct);
...
public sealed class SnapshotService
{
    ...
    public async Task<SnapshotId?> CaptureAsync(CancellationToken ct = default)
    {
        try
        {
            ...
            try
            {
                ...
                var tracked = Paths(await RunAsync("capture", _storage, ["diff-files", "--name-only", "-z", "--", scope], ct));
                var untracked = Paths(await RunAsync("capture", _storage, ["ls-files", "--others", "--exclude-standard", "-z", "--", scope], ct));
                var candidates = tracked.Concat(untracked).Distinct(StringComparer.Ordinal).ToArray();
                var excluded = await IgnoredAsync("capture", candidates, ct);
                ...
                if (remove.Length > 0)
                    await RunAsync("capture", _storage, ["rm", "--cached", "-f", "--ignore-unmatch", "--pathspec-from-file=-", "--pathspec-file-nul"], ct, string.Join('\0', remove) + "\0");
                var stage = candidates.Where(file => !excluded.Contains(file) && !oversized.Contains(file)).ToArray();
                if (stage.Length > 0)
                    await RunAsync("capture", _storage, ["add", "--all", "--sparse", "--pathspec-from-file=-", "--pathspec-file-nul"], ct, string.Join('\0', stage) + "\0");
                return SnapshotId.FromExisting((await RunAsync("capture", _storage, ["write-tree"], ct)).Trim());
            }
    ...
    public async Task<IReadOnlyList<string>> FilesAsync(SnapshotId from, SnapshotId to, CancellationToken ct = default)
    {
        ...
        {
            await InitializeAsync(ct);
            var files = Paths(await RunAsync("files", _storage, ["diff", "--name-only", "--no-renames", "-z", from.Value, to.Value], ct));
            var ignored = await IgnoredAsync("files", files, ct);
        ...
        IReadOnlyList<string>? paths = null, int context = 3, CancellationToken ct = default)
    {
        ...
        {
            await InitializeAsync(ct);
            var changed = Paths(await RunAsync("diff", _storage, ["diff", "--name-only", "--no-renames", "-z", from.Value, to.Value], ct));
            var ignored = await IgnoredAsync("diff", changed, ct);
            var result = new List<FileDiffInfo>();
            foreach (var file in (paths ?? changed).Where(file => !ignored.Contains(file)))
            {
                Absolute(file);
                var status = (await RunAsync("diff", _storage, ["diff", "--name-status", "--no-renames", from.Value, to.Value, "--", file], ct)).Trim();
                var stats = (await RunAsync("diff", _storage, ["diff", "--numstat", "--no-renames", from.Value, to.Value, "--", file], ct)).Split('\t');
                var binary = stats[0] == "-" || stats.ElementAtOrDefault(1) == "-";
                var patch = binary ? "" : await RunAsync("diff", _storage, ["diff", $"--unified={context}", "--no-renames", from.Value, to.Value, "--", file], ct);
    ...
    public async Task RestoreAsync(IReadOnlyDictionary<string, SnapshotId> files, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                var entry = (await RunAsync("restore", _storage, ["ls-tree", "-z", pair.Value.Value, "--", pair.Key], ct)).TrimEnd('\0');
                if (entry.Length > 0)
                {
                    if (!Regex.IsMatch(entry, @"^\d+\s+\w+\s+[0-9a-f]+\t")) throw new SnapshotException("restore", "Invalid tree entry: " + pair.Key);
                    await RunAsync("restore", _storage, ["checkout", pair.Value.Value, "--", pair.Key], ct);
                    continue;
                }
    ...
    private async Task InitializeAsync(CancellationToken ct)
    {
        ...
        var discovery = (await RunAsync("capture", null, ["rev-parse", "--git-dir", "--git-common-dir", "--show-toplevel"], ct)).TrimEnd('\r', '\n').Split('\n').Select(line => line.TrimEnd('\r')).ToArray();
        ...
        {
            await RunAsync("capture", _storage, ["init"], ct);
    ...
    private async Task<HashSet<string>> IgnoredAsync(string operation, IReadOnlyList<string> paths, CancellationToken ct) => paths.Count == 0 ? [] :
        Paths(await RunAsync(operation, _sourceGit, ["check-ignore", "--no-index", "--stdin", "-z"], ct, string.Join('\0', paths) + "\0", allowOne: true)).ToHashSet(StringComparer.Ordinal);
    ...
    private async Task<string> RunAsync(string operation, string? gitDirectory, IReadOnlyList<string> arguments, CancellationToken ct, string? stdin = null, bool allowOne = false)
    {
        ...
        await using var input = new FileStream(temporary, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.Read | FileShare.Delete, 4096, FileOptions.DeleteOnClose);
        if (stdin is not null) { await input.WriteAsync(Encoding.UTF8.GetBytes(stdin), ct); await input.FlushAsync(ct); input.Position = 0; }
    ...
    {
        if (!id.IsInitialized() || !Regex.IsMatch(id.Value, @"\A(?:[0-9a-f]{40}|[0-9a-f]{64})\z")) throw new ArgumentException("Snapshot ID must be a content-addressed Git tree.", nameof(id));
    }
