...
public sealed class WellknownService(WellknownSourceStore store, WellknownTransport transport) : IDisposable
{
    ...
    public async Task<WellknownEntry> AddAsync(string url, CancellationToken ct = default)
    {
        ...
        try
        {
            var entry = await transport.InspectAsync(url, ct);
            ...
            var origins = await store.AddAsync(entry.Origin, ct);
    ...
    public async Task<IReadOnlyList<WellknownEntry>> EntriesAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            foreach (var origin in await store.ReadAsync(ct))
                entries.Add(cache.GetValueOrDefault(origin) ?? await transport.InspectAsync(origin, ct));
    ...
    public async Task<bool> RefreshAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            var origins = await store.ReadAsync(ct);
            if (origins.Length == 0) return false;
            var entries = new List<WellknownEntry>();
            foreach (var origin in origins) entries.Add(await transport.InspectAsync(origin, ct));
    ...
    public async Task RemoveAsync(string url, CancellationToken ct = default)
    {
        ...
        try
        {
            await store.RemoveAsync(origin, ct);
