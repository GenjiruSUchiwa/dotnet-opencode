    ...
    IIntegrationCallbackFactory callbacks, Func<CredentialMutation, CancellationToken, Task> publish)
{
    public async Task<IReadOnlyList<IntegrationDefinition>> DefinitionsAsync(McpRuntime mcp, CancellationToken ct)
    {
        ...
        foreach (var registration in await mcp.OAuthRegistrationsAsync(ct))
        {
    ...
    private async Task<IntegrationAuthorization> ConsoleAsync(FormAnswer? answer, string? label, CancellationToken ct)
    {
        ...
        var attempt = await console.BeginDeviceAuthorizationAsync(server, ct);
        return new(attempt.VerificationUri.AbsoluteUri, $"Enter code: {attempt.UserCode}", "auto", attempt.ExpiresAt,
            async (_, token) => await publish(await console.CompleteDeviceAuthorizationAsync(attempt, label, token), CancellationToken.None),
            () => ValueTask.CompletedTask);
    }

    private async Task<IntegrationAuthorization> HeadlessAsync(FormAnswer? answer, string? label, CancellationToken ct)
    {
        var attempt = await openAi.BeginHeadlessAuthorizationAsync(ct);
        return new(attempt.VerificationUri.AbsoluteUri, $"Enter code: {attempt.UserCode}", "auto", attempt.ExpiresAt,
            async (_, token) => await publish(await openAi.CompleteHeadlessAuthorizationAsync(attempt, label, token), CancellationToken.None),
            () => ValueTask.CompletedTask);
    }
    ...
    private async Task<IntegrationAuthorization> BrowserAsync(FormAnswer? answer, string? label, CancellationToken ct)
    {
        ...
        try { listener = await callbacks.BindAsync(new(Port: 1455, Host: "localhost", Path: "/auth/callback"), ct); }
        catch (IntegrationCallbackAddressInUseException)
        { listener = await callbacks.BindAsync(new(Port: 1457, Host: "localhost", Path: "/auth/callback"), ct); }
        try
        {
            ...
            return new(attempt.AuthorizationUri.AbsoluteUri, "Complete authorization in your browser. This window will close automatically.", "auto", attempt.ExpiresAt,
                async (_, token) =>
                {
                    var callback = await listener.WaitAsync(token);
                    await publish(await openAi.CompleteBrowserAuthorizationAsync(attempt, callback.Code, callback.State, label, token), CancellationToken.None);
                }, listener.DisposeAsync);
        }
        catch { await listener.DisposeAsync(); throw; }
    }
    ...
    {
        var listener = await callbacks.BindAsync(new(registration.RedirectUri, registration.CallbackPort), ct);
        McpOAuthAuthorization? attempt = null;
        try
        {
            // The actual port is bound before SDK discovery/DCR builds the redirect URL.
            attempt = await mcp.StartAuthorizationAsync(registration.Server, listener.RedirectUri, ct);
            var started = attempt;
            ...
            return new(started.AuthorizationUri.AbsoluteUri, $"Authorize {registration.Server} in your browser. This window will close automatically.", "auto", started.ExpiresAt,
                ...
                {
                    var callback = await listener.WaitAsync(token);
                    var selectedLabel = label ?? await IntegrationRuntime.UniqueLabelAsync(credentials, registration.IntegrationId, registration.Server, token);
                    await mcp.CompleteAuthorizationAsync(started, callback.Code, callback.State, callback.Issuer, selectedLabel, token);
                }, async () =>
                {
        ...
        {
            try { if (attempt is not null) await attempt.DisposeAsync(); }
            finally { await listener.DisposeAsync(); }
            throw;
        }
    }
