...
public static class CommandTemplate
{
    ...
    private static readonly Regex Arguments = new("(?:\\[Image[" + Space + "]+[0-9]+\\]|\"[^\"]*\"|'[^']*'|[^" + Space + "\"']+)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
    private static readonly Regex Placeholder = new(@"\$([0-9]+)", RegexOptions.CultureInvariant);
    private static readonly Regex Shell = new("!`([^`]+)`", RegexOptions.CultureInvariant);

    public static string[] ParseArguments(string input) => Arguments.Matches(input)
        .Select(match => Regex.Replace(match.Value, "^[\"']|[\"']$", "")).ToArray();

    internal static string Trim(string value) => Regex.Replace(value, @"\A[" + Space + @"]+|[" + Space + @"]+\z", "");
        ...
        CancellationToken ct = default)
    {
        ...
        var text = Regex.Replace(expanded, Regex.Escape("$ARGUMENTS"), match => ExpandReplacement(input, expanded, match));
        ...
        var outputs = await Task.WhenAll(matches.Select(async match =>
        {
            ...
            try
            {
                return await interpolate(match.Groups[1].Value);
            }
    ...
    private static string ExpandReplacement(string replacement, string source, Match match) =>
        Regex.Replace(replacement, @"\$([$&`'])", token => token.Groups[1].Value switch
        {
